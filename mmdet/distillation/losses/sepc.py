...
class SEPC(nn.Module):
    def __init__(
        ...
        self.lconv2 = sepc_conv(256,
                               down_channels,
                               kernel_size=3,
                               dilation=2,
                               part_deform=lcconv_deform)
                             
        self.lconv3 = sepc_conv(256,
                               down_channels,
                               kernel_size=3,
                               dilation=3,
                               part_deform=lcconv_deform)  
        self.lconv4 = sepc_conv(256,
                               down_channels,
                               kernel_size=3,
                               dilation=4,
                               part_deform=lcconv_deform)                                                                  
    ...
    def forward(self, inputs):
        ...
        loc2 = [self.lconv2(level, item) for level, item in enumerate(x)]
        loc3 = [self.lconv3(level, item) for level, item in enumerate(x)]
        loc4 = [self.lconv4(level, item) for level, item in enumerate(x)]
        
        loc = torch.cat([loc1,loc2,loc3,loc4], dim=2)
