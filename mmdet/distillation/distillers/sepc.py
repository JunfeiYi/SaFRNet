...
class SEPC(nn.Module):
    def __init__(
        ...
        down_channels = 128
        """
        self.lconv1 = nn.Conv2d(256,
                               down_channels,
                               kernel_size=3,
                               dilation=1)
                               
        """
        self.lconv2 = nn.Conv2d(256,
                               down_channels,
                               kernel_size=3,
                               dilation=2,
                               padding=1)
        """                      
        self.lconv3 = nn.Conv2d(256,
                               down_channels,
                               kernel_size=3,
                               dilation=1,
                               )  
        self.lconv4 = nn.Conv2d(256,
                               down_channels,
                               kernel_size=3,
                               dilation=2,
                               padding=1)   
    ...
    def init_weights(self):
        for str in ['conv1', 'conv2', 'conv3', 'conv4']:#
            m = getattr(self, 'l' + str)
    ...
    def forward(self, inputs):
        ...
        loc1 = [self.lconv1(level, item) for level, item in enumerate(x)]
        loc2 = [self.lconv2(level) for level in x]
        #loc3 = [self.lconv3(level) for level in x]
        #loc4 = [self.lconv4(level) for level in x]
        loc=[]
        for i, j, k, l in zip(loc1,loc2,loc3,loc4):
            loc.append(torch.cat([i, j, k, l], dim=1))
        
        if self.iBN:
            #cls = iBN(cls, self.cbn)
            loc = iBN(loc, self.lbn)
        outs = [self.relu(s) for s in loc] + inputs
