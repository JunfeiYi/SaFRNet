...
class DetectionDistiller(BaseDetector):
    ...
    def __init__(self,
        ...
        #self.init_epoch = 8
        #self.max_alpha = 0.00004
        #self.mid_epoch = self.init_epoch + (23 - self.init_epoch) * 0.5
    ...
    def forward_train(self, img, img_metas, **kwargs):
        ...
        student_feats = self.sepc(tuple(student_feats))
        teacher_feats = self.sepc(tuple(teacher_feats))
        ...
        for item_loc in self.distill_cfg:
            ...
            for item_loss in item_loc.methods:
                loss_name = item_loss.name
                student_loss[loss_name] = self.distill_losses[loss_name](student_feat,teacher_feat,kwargs['gt_bboxes'], img_metas) * self.alpha * 2.0
                """
                if self.epoch < self.init_epoch:
                    student_loss[loss_name] = self.distill_losses[loss_name](student_feat,teacher_feat,kwargs['gt_bboxes'], img_metas) * self.alpha
                    
                #elif loss_name is 'loss_fgd_fpn_4' and self.epoch < self.init_epoch:
                #student_loss[loss_name] = self.distill_losses[loss_name](student_feat,teacher_feat,kwargs['gt_bboxes'], img_metas) * self.alpha                  
                elif self.init_epoch <= self.epoch <= self.mid_epoch:
                    alpha_up = (self.epoch - self.init_epoch + 1) / self.mid_epoch * self.max_alpha
                    student_loss[loss_name] = self.distill_losses[loss_name](student_feat,teacher_feat,kwargs['gt_bboxes'], img_metas) * alpha_up
                    
                else:
                    alpha_down = ( 1 - (self.epoch - self.mid_epoch +1 ) / self.mid_epoch ) * self.max_alpha
                    student_loss[loss_name] = self.distill_losses[loss_name](student_feat,teacher_feat,kwargs['gt_bboxes'], img_metas) * alpha_down           
                """
        return student_loss
