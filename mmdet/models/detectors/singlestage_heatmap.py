...
class SingleStageDetectorHP(BaseDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SingleStageDetectorHP, self).__init__(init_cfg)
        if pretrained:
    ...
    def extract_feat(self, img,img_metas):
       ...
       #特征层，图片路径，保存的文件名
        #draw_feature_map1(x,imgpath,name='backbone_') 
        if self.with_neck:
            x = self.neck(x)
        draw_feature_map1(x,imgpath,name='fpn_') 
        return x
    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        #feat = self.bbox_head.forward(feat)
       ...
       #特征层，图片路径，保存的文件名
        ...
        #draw_feature_map1(results_list[1],imgpath,name='head_')       
        bbox_results = [
