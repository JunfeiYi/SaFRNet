...
class GaussKernel2dFn(Function):
    ...
    @staticmethod
    def forward(ctx, input, kernel_size, stride, padding, dilation, channel_wise): # ctx -> self
        ...
        center_xy= ctx.kernel_size[0] * ctx.kernel_size[1] // 2
        cols = F.unfold(input, ctx.kernel_size, ctx.dilation, ctx.padding, ctx.stride)
        cols = cols.view(bs, ch, -1, out_h, out_w) # ctx.kernel_size[0], ctx.kernel_size[1]
        feat_0 = cols.contiguous()[:, :, center_xy:center_xy + 1, :, :]
        cols_var = torch.var(cols, dim=2, keepdim=True)
        #cols_mean = cols_mean.expand_as(cols)
        cols_feat0_n = F.normalize(feat_0, dim=2)
        cols_n = F.normalize(cols, dim=2)
        cos_simi = cols_feat0_n * cols_n
        output = - cos_simi / cols_var
        output = output.view(bs, ch, ctx.kernel_size[0], ctx.kernel_size[1], out_h, out_w)
        if not channel_wise:
    ...
    def backward(ctx, grad_output):
        ...
        center_xy= ctx.kernel_size[0] * ctx.kernel_size[1] // 2
        feat_0 = cols.contiguous()[:, :, center_xy:center_xy + 1, :, :]
        cols_mean = torch.mean(cols, dim=2, keepdim=True)
        cols_var = torch.var(cols, dim=2, keepdim=True)
        cols_feat0_n = F.normalize(feat_0, dim=2)
        cols_n = F.normalize(cols, dim=2)
        N = cols.shape[2]
        cos_simi = cols_n * cols_feat0_n
        grad_var = 1 / (cols_var * cols_var)
        d_var = (2 / N) * (cols - cols_mean.expand_as(cols))  - 2 * (cols - cols_mean.expand_as(cols)) / (N * N)
        s_x2 = (cols * cols).sum(dim=2, keepdim=True)
        d_x_u = s_x2 - (cols * cols)
        d_x = d_x_u / s_x2.pow(1.5) #shape 3 3 he 9 的diff
        d_x = ((d_x * cols_n) / cols_var)#.view(bs, ch, ctx.kernel_size[0], ctx.kernel_size[1], out_h, out_w)
        d_var = (grad_var * cos_simi * d_var)#.view(bs, ch, ctx.kernel_size[0], ctx.kernel_size[1], out_h, out_w)
        grad_output = grad_output.expand_as(cols.view(bs, ch, ctx.kernel_size[0], ctx.kernel_size[1], out_h, out_w)).view(bs, ch, -1, out_h, out_w)
        grad_diff = d_var * grad_output - d_x * grad_output
        grad_diff[:, :, center_xy:center_xy + 1, :, :] -= \
            (d_x * grad_output)[:, :, center_xy:center_xy + 1, :, :]
        print(grad_diff[:, :, center_xy:center_xy + 1, :, :] )
...
def packernel2d(input, mask=None, kernel_size=0, stride=1, padding=0, output_padding=0, dilation=1,
    ...
    if native_impl:
       ...
       # x_var = torch.var(x, dim=2,keepdim=True)
        feat_0 = feat_0.expand_as(x)
        if kernel_type.find('_asym') >= 0:
            x = F.relu(x, inplace=True)
        # x.pow_(2)  # this causes an autograd issue in pytorch>0.4
        if not channel_wise:
            x = torch.sum(x, dim=1, keepdim=True)
            feat_0 = torch.sum(feat_0, dim=1, keepdim=True)
        b0, c0, k0, h0,w0 = x.shape
        x_var = torch.var(x, dim=1 ,unbiased=False, keepdim=True)
        cos_simi = F.normalize(x.contiguous().view(bs,-1), dim=0) * F.normalize(feat_0.contiguous().view(bs, -1), dim=0)
        if kernel_type == 'gaussian':
            cos_simi = cos_simi.contiguous().view(b0, c0, k0, h0, w0)

            cos_simi = -cos_simi / (x_var + 1e-5)#torch.div(cos_simi, x_var)

            cos_simi_w = cos_simi.permute(0, 3, 4, 1, 2)
            cos_simi_w = F.adaptive_avg_pool2d(cos_simi_w.view(b0, -1, c0, k0), (1,1))
            cos_simi_w = torch.softmax(cos_simi_w, dim=0)
            cos_simi_w = cos_simi_w.view(b0, 1, 1, h0, w0)
            #print(cos_simi.shape, cos_simi_w.shape)
            #print(cos_simi_w)
            x = cos_simi_w.expand_as(cos_simi)
            #x = (x - feat_0) * x
            #x = torch.exp_(x.mul_(-0.5)) * x_cos  # TODO profiling for identifying the culprit of 5x slow down
        ###################################################
    ...
    if norm is not None:
        ...
        output_mask = ( ~empty_mask) if output_mask else None
        output_mask=None ############  force to set None
    else:
        output_mask = None
...
class PacConv2d(_PacConvNd):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=1, dilation=1, bias=True,
                 ...
                 filler='uniform', native_impl=True):
        kernel_size = _pair(kernel_size)
        stride = _pair(stride)
        padding = _pair(padding)
        dilation = _pair(dilation)
